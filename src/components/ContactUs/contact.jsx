        ...
        {testimonials.map((testimonial, index) => (
          ...
          >
            <img
              ...
              alt={`Person ${index + 1}`}
              className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-gray-400"
            />
            <h3 className="text-lg md:text-xl">{testimonial.name}</h3>
            <p className="text-xs md:text-sm text-gray-300">
              {testimonial.subtitle}
            </p>
            <p className="text-sm md:text-lg italic max-w-xl mx-auto">
              {testimonial.message}
            </p>
          </div>
        ))}
        ...
        <div className="flex justify-center mt-5">
          {testimonials.map((_, index) => (
            <span
              ...
              className={`h-2 w-2 md:h-2.5 md:w-2.5 mx-1 bg-gray-500 rounded-full cursor-pointer ${
